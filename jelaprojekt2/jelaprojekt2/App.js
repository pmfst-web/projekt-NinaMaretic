...
import povrceReducer from './store/reducers/povrca';
      ...
      <NavigationContainer>
        <Stack.Navigator
          ...
          />
           <Stack.Screen
            name="Drugi"
            component={DrugiEkran}
            options={({ route, navigation }) => {
              const idPovrca = Number(route.params.id_1);
              const povrce = POVRCA.find((p) => p.id_1 === idPovrca);
              return {
                headerTitle: povrce?.namjernica_1,
                headerRight: () => {
                  return (
                    <TouchableOpacity
                      onPress={() => navigation.navigate('Naslovna')}>
